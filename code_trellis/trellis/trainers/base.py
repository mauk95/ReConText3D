...
from .l2sp import make_l2sp_anchor
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        if self.adapter_config is not None and self.adapter_config["name"] == 'LORA':
            ...
            if self.is_master:
                print("LoRA model switched to train mode")
        ...
        if self.is_master:
            ...
            # Write trainer info to log file
            trainer_log_path = os.path.join(self.output_dir, 'trainer_log.txt')
            with open(trainer_log_path, 'a') as f:
                f.write(f"\n[Trainer Initialized] {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write("="*80 + "\n")
                f.write(str(self))
                f.write("\n" + "="*80 + "\n\n")
