...
def init_nodes(save_depth=False, save_normal=False, save_albedo=False, save_mist=False):
    ...
    # Get the active view layer (more robust than hardcoding 'View Layer')
    ...
    # More robust approach using active view layer:
...
def load_object(object_path: str) -> None:
    ...
    ##################
        ...
        # Import PLY with specific settings to preserve vertex colors
        import_function(filepath=object_path)
        # Set up materials to use vertex colors for PLY files
        setup_ply_materials()
    ##################
...
def create_vertex_color_material(obj):
    """Create a material that uses vertex colors for PLY files."""
    has_colors = False
    
    # Check if the object has vertex colors (legacy method)
    if obj.data.vertex_colors:
        ...
        # Set the vertex color layer (use the first one available)
        vertex_color.layer_name = obj.data.vertex_colors[0].name
        
        # Connect nodes: Vertex Color -> BSDF -> Output
        mat.node_tree.links.new(vertex_color.outputs['Color'], bsdf.inputs['Base Color'])
        mat.node_tree.links.new(bsdf.outputs['BSDF'], output.inputs['Surface'])
        
        has_colors = True
        
    # Check for color attributes (newer Blender versions)
    elif hasattr(obj.data, 'color_attributes') and obj.data.color_attributes:
        # Create a new material
        mat = bpy.data.materials.new(name="VertexColorMaterial")
        mat.use_nodes = True
        mat.node_tree.nodes.clear()
        ...
        # Set the color attribute name (use the first one available)
        attr_node.attribute_name = obj.data.color_attributes[0].name
        
        # Connect nodes: Attribute -> BSDF -> Output
    ...
    if has_colors:
        ...
        print(f"[INFO] Created vertex color material for {obj.name}")
    else:
        print(f"[INFO] No vertex colors found for {obj.name}, creating default material")
        # Create a simple default material for objects without vertex colors
...
def setup_ply_materials():
    """Set up materials for PLY files that use vertex colors."""
    for obj in bpy.context.scene.objects:
        if obj.type == 'MESH':
...
def main(arg):
    ...
    for i, view in enumerate(views):
        # if arg.eval_mode:
        #     # place the camera above the object for a positive pitch like 30°, ensuring a proper isometric top-down perspective.
        #     cam.location = (
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_mode', action='store_true', help='Flag for eval mode.')
