...
def main(local_rank, cfg):
    ...
    print(f'Rank {rank}/{world_size}, local rank {local_rank}, node rank {cfg.node_rank}/{cfg.num_nodes}, gpus per node {cfg.num_gpus}')
    ...
    # Load data
    dataset = getattr(datasets, cfg.dataset.name)(cfg.data_dir, metadata_filename=cfg.metadata_filename, **cfg.dataset.args)
    ...
    finetune_ckpt = cfg.finetune_ckpt
    if finetune_ckpt is not None:
        finetune_ckpt = {
            name: finetune_ckpt
            for name, _ in cfg.models.items()
        }
    ...
    trainer = getattr(trainers, cfg.trainer.name)(model_dict, dataset, **cfg.trainer.args, output_dir=cfg.output_dir, 
                                                  load_dir=cfg.load_dir, step=cfg.load_ckpt, num_workers=cfg.num_workers,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metadata_filename', type=str, default='metadata.csv', help='Metadata filename')
    ## dubug
    ...
    parser.add_argument('--master_port', type=str, default='12346', help='Port for distributed training')
    parser.add_argument('--num_workers', type=int, default=0, help='Number of workers for data loading, default to 0')
    parser.add_argument('--finetune_ckpt', type=str, default=None, help='Checkpoint path to finetune from, default to None')
