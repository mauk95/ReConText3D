...
class BasicTrainer(Trainer):
    ...
    def init_models_and_more(self, **kwargs):
        ...
        # ###################
        # print(f"\n🔍 DEBUG: Before master params creation:")
        # print(f"  model_params length: {len(self.model_params)}")
        # print(f"  fp16_mode: {self.fp16_mode}")
        # #####################
        ...
        if self.fp16_mode == 'amp':
            ...
            #############
            # print(f"  Calling make_master_params() with {len(self.model_params)} parameters...")
            self.master_params = make_master_params(self.model_params)
            # print(f"  make_master_params() returned {len(self.master_params)} parameters")

            # #############
            # if len(self.master_params) != len(self.model_params):
            #     print(f"  🚨 PROBLEM: Expected {len(self.model_params)} master params, got {len(self.master_params)}")
            #     print(f"  First few model_params shapes: {[p.shape for p in self.model_params[:5]]}")
            #     print(f"  Master_params shapes: {[p.shape for p in self.master_params]}")
            # ###############
        ...
        #     names = [n for n,p in self.models["denoiser"].named_parameters() if p in set(g['params'])]
        #     # print(f'i: {i}, len(names): {len(names)}, lr: {g.get("lr", None)}, wt_decay: {g.get("weight_decay", None)}')
        #     print(f'names: {names}')
        # ##############################
    ...
    def load(self, load_dir, step=0):
        ...
        # Load L2SP anchor if it exists
        if 'l2sp_anchor' in misc_ckpt:
            self.l2sp_anchor = misc_ckpt['l2sp_anchor']
            if self.is_master:
                total_params = sum(len(anchor) for anchor in self.l2sp_anchor.values())
                print(f"Loaded L2SP anchor with {total_params} parameters across {len(self.l2sp_anchor)} models")
        
        # Restore LoRA adapter config if it exists
        if 'adapter_config' in misc_ckpt:
            loaded_adapter_config = misc_ckpt['adapter_config']
            if self.is_master:
                print(f"Loaded LoRA adapter config: {loaded_adapter_config}")
            # Note: The actual LoRA wrapping will be done in the base class after init_models_and_more()
        del misc_ckpt
        ...
        # Create L2SP anchor after loading base checkpoint weights
        if self.l2sp_anchor is None and self.regularizer_config is not None:
            if self.regularizer_config["name"] == "L2SP":
                self.create_l2sp_anchor()
    ...
    def save(self):
        ...
        # Save L2SP anchor if it exists
        if self.l2sp_anchor is not None:
            misc_ckpt['l2sp_anchor'] = self.l2sp_anchor
        
        # Save LoRA adapter config if using LoRA
        if self.adapter_config is not None and self.adapter_config["name"] == 'LORA':
            misc_ckpt['adapter_config'] = self.adapter_config
    ...
    def finetune_from(self, finetune_ckpt):
        ...
        # Try to load EMA parameters if available (for better fine-tuning performance)
        if self.is_master:
            for i, ema_rate in enumerate(self.ema_rate):
                ema_ckpts = {}
                for name, model in self.models.items():
                    if name in finetune_ckpt:
                        # Try to find corresponding EMA checkpoint
                        base_path = finetune_ckpt[name]
                        if '_step' in base_path:
                            # Replace model checkpoint with EMA checkpoint
                            ema_path = base_path.replace(f'_{name}_step', f'_{name}_ema{ema_rate}_step')
                        else:
                            # If no step pattern, try adding EMA suffix
                            ema_path = base_path.replace('.pt', f'_ema{ema_rate}.pt')
                        
                        if os.path.exists(ema_path):
                            try:
                                ema_ckpt = torch.load(read_file_dist(ema_path), map_location=self.device, weights_only=True)
                                ema_ckpts[name] = ema_ckpt
                                if self.is_master:
                                    print(f'  - Loaded EMA {ema_rate} for {name} from: {ema_path}')
                            except Exception as e:
                                if self.is_master:
                                    print(f'  - Warning: Could not load EMA {ema_rate} for {name}: {e}')
                ...
                if ema_ckpts:
                    ...
                    if self.is_master:
                        print(f'  - No EMA {ema_rate} checkpoints found, using current model weights')
        ...
        if self.regularizer_config is not None:
            if self.regularizer_config["name"] == "L2SP":
                # Create L2SP anchor after loading base checkpoint weights
                self.create_l2sp_anchor()
    ...
    # def save_l2sp_anchor(self, anchor_path=None):
    #     """
    #     Save L2SP anchor separately from training checkpoint.
    #     Should be called only by the rank 0 process.
    #     """
    #     if self.l2sp_anchor is not None and self.is_master:
    #         if anchor_path is None:
    #             anchor_path = os.path.join(self.output_dir, 'l2sp_anchor.pt')
    #         torch.save(self.l2sp_anchor, anchor_path)
    #         total_params = sum(len(anchor) for anchor in self.l2sp_anchor.values())
    #         print(f"Saved L2SP anchor with {total_params} parameters to: {anchor_path}")
    
    # def load_l2sp_anchor(self, anchor_path=None):
    #     """
    #     Load L2SP anchor from separate file.
    #     Should be called by all processes.
    #     """
    #     if anchor_path is None:
    #         anchor_path = os.path.join(self.output_dir, 'l2sp_anchor.pt')
    ...
    #         self.l2sp_anchor = torch.load(anchor_path, map_location=torch.device('cpu'))
    #         if self.is_master:
    #             total_params = sum(len(anchor) for anchor in self.l2sp_anchor.values())
    #             print(f"Loaded L2SP anchor with {total_params} parameters from: {anchor_path}")
    #     else:
    #         if self.is_master:
    #             print(f"L2SP anchor file not found: {anchor_path}")
    ...
    def _rebuild_optimizer_for_lora(self):
        ...
        # Update DDP wrapper for LoRA model
        if self.world_size > 1:
            self.training_models["denoiser"] = DDP(
            ...
            self.training_models["denoiser"] = self.models["denoiser"]
        
        # Rebuild model params with only trainable LoRA parameters
        self.model_params = sum(
            [[p for p in model.parameters() if p.requires_grad] for model in self.models.values()]
        , [])
        
        if self.is_master:
            print(f"📊 New trainable parameters: {len(self.model_params)}")
        
        # Rebuild master params for FP16
        if self.fp16_mode == 'amp':
            self.master_params = self.model_params
        elif self.fp16_mode == 'inflat_all':
            from .utils import make_master_params
            self.master_params = make_master_params(self.model_params)
            self.fp16_scale_growth = self.fp16_scale_growth
            self.log_scale = 20.0
        elif self.fp16_mode is None:
        ...
        # Rebuild EMA params (only for master process)
        if self.is_master:
            self.ema_params = [copy.deepcopy(self.master_params) for _ in self.ema_rate]

        # Rebuild optimizer with new LoRA parameters
        if hasattr(torch.optim, self.optimizer_config['name']):
            self.optimizer = getattr(torch.optim, self.optimizer_config['name'])(self.master_params, **self.optimizer_config['args'])
        else:
            self.optimizer = globals()[self.optimizer_config['name']](self.master_params, **self.optimizer_config['args'])
        
        # Rebuild learning rate scheduler if it exists
        if self.lr_scheduler_config is not None:
            if hasattr(torch.optim.lr_scheduler, self.lr_scheduler_config['name']):
                self.lr_scheduler = getattr(torch.optim.lr_scheduler, self.lr_scheduler_config['name'])(self.optimizer, **self.lr_scheduler_config['args'])
            else:
        ...
        if self.is_master:
            print("✅ Successfully rebuilt optimizer for LoRA training")
