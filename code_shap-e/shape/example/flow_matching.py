...
# from ...trainers.l2sp import l2sp_loss
from ..l2sp import l2sp_loss, l2sp_loss_new, l2sp_loss_for_model
...
class FlowMatchingTrainer(BasicTrainer):
    ...
    def training_losses(
        ...
        **kwargs
        ...
        # Add L2SP regularization penalty if enabled
        if self.l2sp_anchor is not None and self.regularizer_config is not None:
            lambda_main = self.regularizer_config["args"].get('lambda_main', 0.0)
            lambda_embed = self.regularizer_config["args"].get('lambda_embed', 0.0)
            if lambda_main > 0.0:
                l2sp_penalty = torch.tensor(0.0, device=pred.device, dtype=pred.dtype)
                for name, model in self.models.items():
                    if name in self.l2sp_anchor:
                        # if lambda_embed != lambda_main:
                        #     penalty = l2sp_loss_new(model, self.l2sp_anchor[name], lambda_main=lambda_main, lambda_embed=lambda_embed)
                        # else:
                        #     penalty = l2sp_loss(model, self.l2sp_anchor[name], lambda_l2sp=lambda_main)
                        penalty = l2sp_loss_for_model(model, self.l2sp_anchor[name], self.step, self.regularizer_config["args"])
                        l2sp_penalty = l2sp_penalty + penalty
                
                terms["l2sp_penalty"] = l2sp_penalty
                terms["loss"] = terms["loss"] + l2sp_penalty
